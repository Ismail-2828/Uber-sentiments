...
model = joblib.load('Uber_model.pkl')
vectorizer = joblib.load('Uber_vectorizer.pkl')
stopwords = nltk.corpus.stopwords.words("English")

neg_df = pd.DataFrame(columns=["Review"]) 
...
def predict():
    ...
    if request.method == 'POST':
        ...
        if prediction == "Negative":
           global neg_df
           neg_df = neg_df.append({"Negative Reviews":review},ignore_index=True)
           neg_df.to_csv("Negative_reviews",index=False)
